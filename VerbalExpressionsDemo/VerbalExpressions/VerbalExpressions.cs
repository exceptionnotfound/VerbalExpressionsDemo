...
namespace CSharpVerbalExpressions
{
    public class VerbalExpressions
    {
        ...
        public VerbalExpressions Quantifier(int number)
        {
            if (number <= 0)
            {
                throw new ArgumentException("The quantifier must be greater than 0.");
            }

            var quantifier = string.Format("{{{0}}}", number);

            return Add(quantifier, false);
        }
